<template>
  <div class="kl8-statistics" v-loading="loading" element-loading-text="正在分析数据..." element-loading-background="rgba(10,10,20,0.8)">
    <!-- 顶部控制条 -->
    ...
    <el-row :gutter="16" v-if="stats" class="chart-row">
      <el-col :span="12">
        <div class="glass-card chart-card chart-important">
          <div class="chart-title"><span class="title-dot dot-blue"></span><h3>号码出现频次 (01-80)</h3><span class="chart-badge blue">80选20</span></div>
          <div ref="numberFreqRef" class="chart-box"></div>
        </div>
      </el-col>
      <el-col :span="12">
        <div class="glass-card chart-card chart-important">
          <div class="chart-title"><span class="title-dot dot-orange"></span><h3>号码遗漏值 (当前未出现期数)</h3></div>
          <div ref="numberOmitRef" class="chart-box"></div>
        </div>
      </el-col>
    </el-row>

    <!-- 分区2: 冷热号与分区 -->
    <div class="section-header" v-if="stats"><span class="section-line"></span><h4>冷热号 &amp; 分区分析</h4><span class="section-line"></span></div>
    <el-row :gutter="16" v-if="stats" class="chart-row">
      <el-col :span="12">
        <div class="glass-card chart-card">
          <div class="chart-title"><span class="title-dot dot-red"></span><h3>冷热号对比 (热号 Top10 / 冷号 Top10)</h3></div>
          <div ref="hotColdRef" class="chart-box"></div>
        </div>
      </el-col>
      <el-col :span="12">
        <div class="glass-card chart-card">
          <div class="chart-title"><span class="title-dot dot-cyan"></span><h3>四区比 Top10</h3><span class="chart-badge gray">1-20 / 21-40 / 41-60 / 61-80</span></div>
    ...
    <!-- 分区3: 形态分布 -->
    <div class="section-header" v-if="stats"><span class="section-line"></span><h4>形态分布</h4><span class="section-line"></span></div>
    <el-row :gutter="16" v-if="stats" class="chart-row">
      <el-col :span="8">
      ...
      <el-col :span="8">
        <div class="glass-card chart-card chart-pie">
          <div class="chart-title"><span class="title-dot dot-orange"></span><h3>大小比分布 (1-40 小 / 41-80 大)</h3></div>
    ...
    <el-row :gutter="16" v-if="stats" class="chart-row">
      ...
      <el-col :span="12">
        <div class="glass-card chart-card">
          <div class="chart-title"><span class="title-dot dot-green"></span><h3>重号 / 连号统计</h3></div>
          <div ref="repeatConsecRef" class="chart-box"></div>
        </div>
      </el-col>
    </el-row>
    ...
    <el-row :gutter="16" v-if="stats" class="chart-row">
      <el-col :span="6">
        <div class="glass-card rank-card rank-hot">
          <div class="chart-title"><span class="title-dot dot-red"></span><h3>热号 Top10</h3></div>
          <el-table :data="stats.hotTop10" size="small" class="dark-table">
      ...
      <el-col :span="6">
        <div class="glass-card rank-card rank-cold">
          <div class="chart-title"><span class="title-dot dot-blue"></span><h3>冷号 Top10</h3></div>
          <el-table :data="stats.coldTop10" size="small" class="dark-table">
      ...
      <el-col :span="6">
        <div class="glass-card rank-card rank-summary">
          <div class="chart-title"><span class="title-dot dot-orange"></span><h3>重号统计</h3></div>
      ...
      <el-col :span="6">
        <div class="glass-card rank-card rank-summary">
          <div class="chart-title"><span class="title-dot dot-green"></span><h3>连号统计</h3></div>
          <div class="repeat-info">
            <div class="repeat-row"><span>含连号期数</span><b>{{ stats.consecutiveStats.hasConsecutiveCount }}</b></div>
            ...
            <div class="repeat-row"><span>连号概率</span><b>{{ stats.consecutiveStats.consecutiveRate }}%</b></div>
          </div>
        </div>
      </el-col>
    </el-row>
...
const numberFreqRef = ref(null)
const numberOmitRef = ref(null)
const hotColdRef = ref(null)
...
const repeatConsecRef = ref(null)
...
function initCharts() {
  ...
  charts.length = 0
  const refs = [numberFreqRef, numberOmitRef, hotColdRef, zoneRef, oddEvenRef, bigSmallRef, primeCompRef, sumRef, spanRef, acRef, repeatConsecRef]
  refs.forEach(r => {
...
async function loadStats() {
  loading.value = true
  try {
    const data = await kl8.statistics(recentPeriods.value)
    stats.value = data
    await nextTick()
...
function renderCharts() {
  ...
  // 1. 号码频次 (01-80) - 按号码升序
  const freqEntries = sortNumberMap(d.numberFreq)
  setChart(0, {
    tooltip: { trigger: 'axis', backgroundColor: darkTheme.tooltipBg, textStyle: { color: '#fff' } },
    grid: { left: 50, right: 20, top: 30, bottom: 50 },
    xAxis: { type: 'category', data: freqEntries.map(e=>e[0]), axisLabel: { color: darkTheme.textColor, fontSize: 9, rotate: 45 }, axisLine: { lineStyle: { color: darkTheme.axisLineColor } } },
    yAxis: { type: 'value', axisLabel: { color: darkTheme.textColor }, splitLine: { lineStyle: { color: darkTheme.splitLineColor } } },
    series: [{ name: '频次', type: 'bar', data: freqEntries.map(e=>Number(e[1])),
  ...
  // 2. 号码遗漏值 (01-80) - 按号码升序
  const omitEntries = sortNumberMap(d.numberOmit)
  setChart(1, {
    tooltip: { trigger: 'axis', backgroundColor: darkTheme.tooltipBg, textStyle: { color: '#fff' } },
    grid: { left: 50, right: 20, top: 30, bottom: 50 },
    xAxis: { type: 'category', data: omitEntries.map(e=>e[0]), axisLabel: { color: darkTheme.textColor, fontSize: 9, rotate: 45 }, axisLine: { lineStyle: { color: darkTheme.axisLineColor } } },
    yAxis: { type: 'value', axisLabel: { color: darkTheme.textColor }, splitLine: { lineStyle: { color: darkTheme.splitLineColor } } },
    series: [{ name: '遗漏期数', type: 'bar', data: omitEntries.map(e=>Number(e[1])),
  ...
  // 3. 冷热号对比双向柱状图 - 空值保护
  const hotTop = Array.isArray(d.hotTop10) ? d.hotTop10.filter(c => Number(c.freq) > 0) : []
  const coldTop = Array.isArray(d.coldTop10) ? d.coldTop10.filter(c => Number(c.freq) > 0) : []
  const hotCodes = hotTop.map(c => c.code)
  const coldCodes = coldTop.map(c => c.code)
  const hotData = hotTop.map(c => Number(c.freq))
  const coldData = coldTop.map(c => -Number(c.freq))
  if (hotCodes.length + coldCodes.length > 0) {
    setChart(2, {
      tooltip: { trigger: 'axis', backgroundColor: darkTheme.tooltipBg, textStyle: { color: '#fff' },
      ...
      legend: { data: ['热号', '冷号'], textStyle: { color: darkTheme.textColor }, top: 0 },
      grid: { left: 40, right: 30, top: 40, bottom: 20 },
      xAxis: { type: 'value', axisLabel: { color: darkTheme.textColor, formatter: function(v){ return Math.abs(v) } }, splitLine: { lineStyle: { color: darkTheme.splitLineColor } } },
      yAxis: { type: 'category', data: [...hotCodes, ...coldCodes], axisLabel: { color: darkTheme.textColor }, axisLine: { lineStyle: { color: darkTheme.axisLineColor } } },
      series: [
        { name: '热号', type: 'bar', data: [...hotData, ...new Array(coldCodes.length).fill(0)],
          itemStyle: { color: new echarts.graphic.LinearGradient(1,0,0,0, [{offset:0,color:'#ff6b35'},{offset:1,color:'#ff9966'}]) } },
        { name: '冷号', type: 'bar', data: [...new Array(hotCodes.length).fill(0), ...coldData],
          itemStyle: { color: new echarts.graphic.LinearGradient(0,0,1,0, [{offset:0,color:'#4a90e2'},{offset:1,color:'#66ccff'}]) } }
      ]
    })
  }
  ...
  setChart(3, {
    ...
    series: [{ name: '出现次数', type: 'bar', data: zoneData.map(z=>Number(z.count)),
      itemStyle: { color: new echarts.graphic.LinearGradient(1,0,0,0, [{offset:0,color:'#4a90e2'},{offset:1,color:'#66ccff'}]) },
  ...
  setChart(9, {
    ...
    series: [{ name: '期数', type: 'bar', data: acHist.map(h=>Number(h.count)),
      itemStyle: { color: new echarts.graphic.LinearGradient(0,0,0,1, [{offset:0,color:'#ff6b35'},{offset:1,color:'#ffd666'}]) },
      markLine: { data: [{ type: 'average', name: '平均' }], lineStyle: { color: '#ffd666' } } }]
  })

  // 11. 重号 / 连号统计 (双饼图) - 空值保护
  const rs = d.repeatStats || {}
  const cs = d.consecutiveStats || {}
  const total = Number(d.totalDraws) || 0
  const noRepeat = Math.max(0, total - Number(rs.hasRepeatCount || 0))
  const leftPie = [
    { name:'含重号', value: Number(rs.hasRepeatCount) || 0, itemStyle:{color:'#ff6b35'} },
    { name:'无重号', value: noRepeat, itemStyle:{color:'#4a90e2'} }
  ].filter(x => x.value > 0)
  const rightPie = [
    { name:'含连号', value: Number(cs.hasConsecutiveCount) || 0, itemStyle:{color:'#ffd666'} },
    { name:'无连号', value: Number(cs.noConsecutiveCount) || 0, itemStyle:{color:'#66ccff'} }
  ].filter(x => x.value > 0)
  setChart(10, {
    tooltip: { trigger: 'item', backgroundColor: darkTheme.tooltipBg, textStyle: { color: '#fff' } },
    legend: { textStyle: { color: darkTheme.textColor }, bottom: 0 },
    series: [
      { name: '重号', type: 'pie', radius: ['20%','40%'], center: ['25%','45%'],
        data: leftPie, label: { color: darkTheme.textColor } },
      { name: '连号', type: 'pie', radius: ['20%','40%'], center: ['75%','45%'],
        data: rightPie, label: { color: darkTheme.textColor } }
    ]
  })
}
...
.kl8-statistics { max-width: 1700px; margin: 0 auto; min-height: 200px; }
...
.chart-important {
  ...
  background: linear-gradient(135deg, rgba(25,30,50,0.6), rgba(18,20,38,0.4));
}
...
.dot-green { background: #66cc66; box-shadow: 0 0 8px rgba(102,204,102,0.5); }
